                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="space-y-1">
                        ...
                        {environment.driver === "ssh" ? (
                          ...
                          <div className="text-xs text-muted-foreground">Runs on this Artu Teams host.</div>
                        )}
                      </div>
            ...
            <div className="space-y-3">
              ...
              {environmentForm.driver === "ssh" ? (
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <Field label="Remote workspace path" hint="Absolute path that Artu Teams will verify during SSH connection tests.">
                    <input
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                      type="text"
              ...
              {environmentForm.driver === "sandbox" ? (
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    <ToggleField
                      label="Reuse lease"
                      hint="When enabled, Artu Teams will try to reconnect to a previously leased sandbox before provisioning a new one."
...
function buildAgentSnippet(input: AgentSnippetInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask your user to configure a reachable hostname in Artu Teams, then retry.
Suggested steps:
- choose a hostname that resolves to the Artu Teams host from your runtime
- run: pnpm paperclipai allowed-hostname <host>
- restart Artu Teams
- verify with: curl -fsS http://<host>:3100/api/health
- regenerate this invite snippet`
      : `If none are reachable, ask your user to add a reachable hostname in Artu Teams, restart, and retry.
Suggested command:
- pnpm paperclipai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nYou MUST test Artu Teams-to-gateway reachability, call: ${resolutionTestUrl}?url=<urlencoded-gateway-url> (using the hostname that worked above). Do not assume your 172.x is necessarily reachable from Artu Teams. Test it. `
    : "";

  return `You're invited to join an Artu Teams organization.

The URLs you should try are:
${candidateList}

Connectivity guidance:
If you are running on a different machine than Artu Teams, Artu Teams must be reachable at one of the hostnames used above.
Verify the hostname works from your runtime with: GET <base-url>/api/health

${connectivityBlock}

For your "agentName", just use your own agent name (OpenClaw or whatever your name is) and send it directly.
You also need to send a gateway URL that Artu Teams can access. Check your config and include that URL.${resolutionLine}
...
- Keep device auth enabled (recommended). If \`devicePrivateKeyPem\` is omitted, Artu Teams will generate and persist one during join so pairing approvals remain stable.
...
Then after you've connected to Artu Teams (exchanged keys etc.) you MUST review and follow the onboarding instructions in onboarding.txt they give you.
